...
resource "aws_security_group" "rds" {
  ...
  description = "PostgreSQL access from ECS and MWAA"
  vpc_id      = aws_vpc.main.id
  ...
  ingress {
    ...
    to_port         = 5432
    protocol        = "tcp"
    security_groups = [aws_security_group.ecs.id, aws_security_group.mwaa.id]
  }
  egress {
    from_port   = 0
    to_port     = 0
...
resource "aws_security_group" "kafka" {
  ...
  description = "MSK Kafka access from ECS and MWAA"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "opensearch" {
  ...
  description = "OpenSearch access from ECS and MWAA"
  vpc_id      = aws_vpc.main.id
...
resource "aws_security_group" "mwaa" {
  ...
  description = "MWAA Airflow environment"
  vpc_id      = aws_vpc.main.id
...
# ─── VPC Endpoints (reduce NAT costs for S3, ECR, SageMaker) ─────────────────
